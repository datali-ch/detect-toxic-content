...
def main(args: argparse.Namespace) -> None:
    ...
    for model_name, file, function in zip(models, files, functions):
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            features, df[LABELS], test_size=TEST_SIZE, random_state=123
        )
        model = NbSvmClassifier(C=C)
        _, metrics_test, measures = fitModel(
...
def get_arg_parser():
    parser = argparse.ArgumentParser(
        description="This script addresses the problem of detecting toxic text content."
    )
